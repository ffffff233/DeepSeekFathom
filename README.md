...
```powershell
py -3 -m pip install --upgrade "deepseek-tulagent[desktop] @ https://github.com/ffffff233/deepseek-tulagent/archive/refs/tags/v0.1.95.tar.gz"
deepseekTulDesktop
```
...
```powershell
py -3 -m pip install --upgrade https://github.com/ffffff233/deepseek-tulagent/archive/refs/tags/v0.1.95.tar.gz
deepseekTul config set --base-url https://api.deepseek.com --api-key sk-... --model deepseek-v4-flash
deepseekTul doctor --live
deepseekTul
```
...
```bash
python3 -m pip install --upgrade https://github.com/ffffff233/deepseek-tulagent/archive/refs/tags/v0.1.95.tar.gz
```
...
| `DSTUL_SEARCH_URL` | auto-detect localhost | Local search engine endpoint, e.g. SearXNG `/search` or YaCy `/yacysearch.json`; result-page fetching respects robots.txt |
...
- `web_search`: query a local SearXNG/YaCy-compatible search engine; supports `search_url`, `language`, `categories`, `time_range`, and opt-in `fetch_pages` with robots.txt checks and short excerpts
