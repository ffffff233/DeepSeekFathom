...
- **新增：搜索合规护栏**。结果页抓取默认关闭；显式开启 `fetch_pages` 或直读 URL 时会检查 `robots.txt`，被禁止时跳过；正文摘录默认缩短并设置硬上限，避免大段搬运网页内容。
- **优化：搜索失败诊断更明确**。本地搜索没启动或没结果时会列出尝试过的本地端点，并提示配置 `DSTUL_SEARCH_URL`，不再给一堆第三方 HTML 解析失败。
- **同步：包版本和 README 安装链接更新到 `v0.1.95`**。
...
- **Added: compliance guardrails for search**. Result-page fetching is off by default; explicit `fetch_pages` and direct URL reads check `robots.txt`, skip blocked pages, and keep excerpts short with a hard cap.
...
- **Synced: package version and README install links are now `v0.1.95`**.
