...
| `DSTUL_SEARCH_URL` | 自动探测本机端口 | 本地搜索引擎地址，例如 SearXNG `/search` 或 YaCy `/yacysearch.json`；结果页抓取会检查 robots.txt |
...
- `search_text`：本地文本搜索
- `web_search`：调用本地 SearXNG/YaCy 兼容搜索引擎；支持 `search_url`、`language`、`categories`、`time_range`，可选 `fetch_pages` 会检查 robots.txt 且只返回短摘录
